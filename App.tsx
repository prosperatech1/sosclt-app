...
// --- COMPONENTE RAIZ (Gerencia Sessão e Roteamento) ---
  ...
  if (isLoading) {
    return (
      <View style={styles.loadingContainer}>
        ...
        <Text style={{ color: '#fff', marginTop: 10 }}>Carregando SOSCLT...</Text>
      </View>
    );
  }
...
function AuthScreen() {
  ...
  return (
    <View style={styles.authContainer}>
      ...
      <TextInput style={styles.input} placeholder="Senha" value={password} onChangeText={setPassword} secureTextEntry placeholderTextColor="#666" />
...
// --- TELA PRINCIPAL (SOS + GRAVAÇÃO + HISTÓRICO) ---
function HomeScreen({ session }: { session: any }) {
  ...
  // ✅ FUNÇÃO CORRIGIDA (com pedido explícito de permissão)
  const startRecording = async () => {
    try {
      const { status: audioStatus } = await Audio.requestPermissionsAsync();
      if (audioStatus !== 'granted') {
        Alert.alert('Permissão Bloqueada', 'Vá em Configurações > Apps > Expo Go > Permissões e ative o Microfone.');
        return;
      }
      ...
      Alert.alert('Erro no Microfone', err.message || 'Falha técnica ao iniciar gravação.');
    }
  };
  ...
  const renderRecording = ({ item }: { item: Recording }) => (
    <View style={styles.recItem}>
      ...
      <View style={styles.recActions}>
        ...
        <TouchableOpacity style={styles.actBtn} onPress={() => Sharing.shareAsync(item.uri, { mimeType: 'audio/mp4' }).catch(() => Alert.alert('Erro', 'Falha ao compartilhar.'))}>
  ...
  return (
    <View style={styles.container}>
      ...
      <TouchableOpacity style={styles.folderBtn} onPress={openHistory}>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              ...
              <TouchableOpacity onPress={() => setShowHistory(false)}><Text style={styles.closeText}></Text></TouchableOpacity>
            </View>
